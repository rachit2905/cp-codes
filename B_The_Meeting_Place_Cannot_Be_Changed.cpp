...
bool check(double mid, vector<double> v1, vector<double> v2)
{
    vector<pair<double, double>> v3;
    l1 n = v1.size();
    for (l1 i = 0; i < n; i++)
    {
        v3.pb({v1[i] - v2[i] * mid, v1[i] + v2[i] * mid});
    }
    double x = v3[0].first, y = v3[0].second;
    for (l1 i = 1; i < n; i++)
    {
        x = max(x, v3[i].first);
        y = min(y, v3[i].second);
...
void solve()
{
    l1 n;
    cin >> n;
    vector<double> v1(n, 0.0), v2(n, 0.0);
    for (l1 i = 0; i < n; i++)
    {
        cin >> v1[i];
    }
    ...
    if (check(l, v1, v2))
        cout << setprecision(8) << fixed << l << ln;
    else
        cout << setprecision(8) << fixed << r << ln;
}
int main()
{
    fast_cin();
