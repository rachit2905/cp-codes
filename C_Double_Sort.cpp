...
void solve()
{
    ...
    for (l1 i = 0; i < n; i++)
    {
        if (v1[i] != v3[i].first || v2[i] != v3[i].second)
        {
            for (l1 j = i + 1; j < n; j++)
            {
                if (v1[j] == v3[i].first && v2[j] == v3[i].second)
                {
                    swap(v1[i], v1[j]);
                    swap(v2[i], v2[j]);
                    v4.pb({i + 1, j + 1});
    ...
    for (l1 i = 1; i < n; i++)
    {
        if (v3[i].first < v3[i - 1].first || v3[i].second < v3[i - 1].second)
        {
            co = 0;
            break;
        }
    ...
    {
        out(v4.size());
        for (l1 i = 0; i < v4.size(); i++)
        {
            cout << v4[i].first << " " << v4[i].second << ln;
        }
    }
}
int main()
